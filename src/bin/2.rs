...
fn get_passwords(file: &str) -> Vec<PasswordPolicy>{
    ...
    r.captures_iter(&get_resource(file)).map(|cap|{
        PasswordPolicy {
            range: (cap[1].parse::<usize>().unwrap(), cap[2].parse::<usize>().unwrap()),
...
fn main() {
    let valid_passwords = get_passwords("day2-part1").iter().filter(|x| x.validate()).count();
