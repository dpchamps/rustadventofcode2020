...
fn extract_gaps(sorted_list: &Vec<i32>) -> Vec<(usize, i32)> {
    sorted_list.iter().enumerate().fold((vec![], -1), |(mut candidates, last), (idx, &el)| {
        if idx == 0 { return (candidates, el) }
        if el - last > 1{
...
fn find_valid_gap(list: &Vec<String>) -> Option<i32> {
    ...
    extract_gaps(&sortable).iter().find(|(idx, el)|{
        match (sortable.get(*idx), sortable.get(*idx+1)) {
...
#[cfg(test)]
mod day_5_tests{
    use crate::{BinaryRange, reduce_rows, reduce_seats, get_seat_id, find_max};
